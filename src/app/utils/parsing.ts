...
async function get_content(url:string) {
  const crawler = new Crawler()
  const page = await crawler.crawl(url)
  const timestampAsString = generateTimestampAsString();
  const page_content = timestampAsString + page![0].content
  console.log(page_content)
}


function generateTimestampAsString(): string {
  const now = new Date();
